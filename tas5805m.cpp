...
esp_err_t tas5805m::begin(tas5805m_mode mode)
{
    int ret;
    ...
    ret = _write_byte(TAS5805M_DEVICE_CTRL_2_REGISTER, 0x02);
    vTaskDelay(100 / portTICK_RATE_MS);
    if (ret != ESP_OK) return ret;

    log_i("Setting to PLAY");
    ret = _write_byte(TAS5805M_DEVICE_CTRL_2_REGISTER, 0x03);
    if (ret != ESP_OK) return ret;
...
{
    uint8_t value = 0;
    int ret = _read_byte(TAS5805M_AGAIN_REGISTER, &value);
    *volume = value << 3;
    return ret;
}
...
{
    uint8_t value = 0;
    int ret = _read_byte(TAS5805M_DIG_VOL_CTRL_REGISTER, &value);
    *volume = value;
    return ret;
}
