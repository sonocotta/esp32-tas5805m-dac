...
- Read DAC state
  ...
  - Get power state and automute state
...
The `tas5805m.h` is a low level driver with minimum dependencies (it used only esp-idf). Lack of dependencies comes with a small price: few things need to be initialized prior to driver initialisation, or it will fail. More specifically, driver will expect
- I2C communication initialized and ready. It will use esp-idf communication functions, assuming I2C is ready.
- I2S peripheral is initialized and ready. DAC require to see an I2S clock for a few milliseconds, or it will ignore many of the DSP settings.

I've implemented a `tas5805m.hpp` wrapper, that I'm primarily use in the Arduino code, so it becomes pretty easy:
...
Next, I'm planning to implement esp-adf component, that usually implements I2C driver initialisation.
...
To set the volume, use the `tas5805m_set_volume` function (volume is in the range [TAS5805M_VOLUME_MIN..TAS5805M_VOLUME_MAX] which is [0..158], default 110, 0 being a full mute):
...
`158` is a bit weird number, but it is actually representation of 2Db steps within possible range of this DAC
...
Default is PLAY state and it will keep in PLAY forever unless specifically told to
...
Mute is a special case of a power state, but sinci it will be typically used to shut doen the amp when idling, I exposed it 
...
|  | BTL (default, STEREO) | PBTL (MONO, double power) |
|---|---|---|
...
The DAC should auto-detect sample rate and BCK clock frequency. I added these functinos purely for debugging issues (in case you switch them for any reason).
...
Power state pretty much represents the value from the `tas5805m_get_state` function, but it is read directly from the DAC register, so it would be more precise state (affected by power glitches, auto-shutdown, fault or any other reason)
...
```
DAC will auto-mute channels individually, when no input signal is detected. his will give direct access to mute states per channel.
...
DAc has a powerfull fault detection system, hat allows to self-diagnose issues with power, data signal, short ircuits, overheating etc.

General pattern is, one should implement periodic chek of the fault registers, and in case there are any - act accorgingly and reset the faults afterwards. Here is the possible pattern impelemntation:
...
This library is based on the ESP-IDF framework and the TAS5805M DAC documentation. I also used some paert of the [Linux Kernel implementaion](https://github.com/torvalds/linux/blob/master/sound/soc/codecs/tas5805m.c) of the driver, and also [my own implmentation](https://github.com/sonocotta/tas5805m-driver-for-raspbian)
