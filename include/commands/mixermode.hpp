...
class MixerModeCommand: public Command
{
...
private:
    ...
    static int mixer_mode_command_handler(int argc, char **argv)
    {
        int nerrors = arg_parse(argc, argv, (void **)&mixer_mode_args);
        if (nerrors > 0)
        {
            arg_print_errors(stderr, mixer_mode_args.end, "mixer");
            ESP_LOGE(TAG, "Invalid command usage");
            return 1;
        }

        if (mixer_mode_args.action->count == 0)
        {
        ...
        MixerMode mode = get_mixer_mode(mixer_mode_args.action->sval[0]);
...
public:
    ...
    static MixerModeArgs mixer_mode_args;
    ...
    esp_console_cmd_t getCommand()
    {
        return {
            ...
            .hint = NULL,
            .func = &mixer_mode_command_handler,
            .argtable = &mixer_mode_args
        };
    }
};